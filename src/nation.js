...
module.exports = class Nation {
  ...
  constructor(options, config) {
    ...
    this._militaryPopulation = new Scout(config.getProp('INITIAL_MILITARY_POPULATION'), 1000);
    this._population = new Scout(config.getProp('INITIAL_POPULATION'), 10000);
    this._education = new Scout(config.getProp('INITIAL_POPULATION'), 1);
    this._health = new Scout(config.getProp('INITIAL_HEALTH'), 1);
    this._balance = config.getProp('INITIAL_BALANCE');
